...
export class NaverMarketAdapter implements MarketAdapter {
  ...
  async execute(product: CatalogProduct, command: ParsedCommand): Promise<MarketExecutionResult> {
    ...
    switch (command.action) {
      case "SET_OUT_OF_STOCK":
        ...
        break;
      case "SET_STOCK": {
        if (command.value === null || !Number.isInteger(command.value) || command.value < 0 || command.value > 99_999_999) {
          throw new Error("네이버 재고는 0~99,999,999 범위의 정수로 입력해 주세요.");
        }
        this.assertBaseStockSafe(beforeSnapshot);
        const statusType = command.value === 0
          ? "OUTOFSTOCK"
          : beforeSnapshot.rawStatus === "SUSPENSION" ? "SUSPENSION" : "SALE";
        await this.changeStatus(token, originProductNo, { statusType, stockQuantity: command.value });
        break;
      }
      case "SET_PRICE":
        if (command.value === null || !Number.isInteger(command.value) || command.value < 1 || command.value > 999_999_990) {
          throw new Error("네이버 판매가는 1~999,999,990원 범위의 정수로 입력해 주세요.");
        }
        ...
        break;
      case "STOP_SALES":
        await this.changeStatus(token, originProductNo, { statusType: "SUSPENSION" });
        break;
      case "RESUME_SALES":
        if (beforeSnapshot.optionStockManaged) {
          throw new Error("이 네이버 상품은 옵션별 재고관리 상품입니다. 옵션별 실제 재고를 확인할 수 있을 때까지 자동 판매재개를 차단합니다.");
        }
        if (before.stock === 0) {
          throw new Error("네이버 판매재개 전에 재고를 1개 이상 설정해 주세요.");
        }
  ...
  private async snapshot(product: CatalogProduct): Promise<NaverSnapshot> {
    const token = await this.token(product.tenantId);
    return this.snapshotWithToken(product, token);
  }
  ...
  private async snapshotWithToken(product: CatalogProduct, token: string): Promise<NaverSnapshot> {
    ...
    const origin = objectValue(root.originProduct);
    if (!Object.keys(origin).length) throw new Error("네이버 원상품 조회 응답에 originProduct가 없습니다.");

    const rawStatus = stringValue(origin.statusType) ?? "UNKNOWN";
    const price = finiteNumber(origin.salePrice);
    const stock = finiteNumber(origin.stockQuantity);
    const detailAttribute = objectValue(origin.detailAttribute);
    const optionInfo = objectValue(detailAttribute.optionInfo);
    ...
    return {
      ...
      state: {
        market: this.market,
        externalId: originProductNo,
        price,
        stock,
        saleStatus: mapStatus(rawStatus, stock)
      }
    };
  }

  private assertBaseStockSafe(snapshot: NaverSnapshot) {
    if (snapshot.optionStockManaged) {
      throw new Error("이 네이버 상품은 옵션별 재고관리 상품입니다. 옵션 재고 API 실계정 검증 전에는 전체 재고 자동변경을 차단합니다.");
    }
  }

  private async changeStatus(token: string, originProductNo: string, body: Record<string, unknown>) {
    await this.request(token, `/v1/products/origin-products/${encodeURIComponent(originProductNo)}/change-status`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  private async token(tenantId: string): Promise<string> {
    ...
    try {
      const response = await this.fetchImpl(`${this.apiBase}/v1/oauth2/token`, {
        method: "POST",
...
function productNo(product: CatalogProduct): string {
  const value = (product.markets.naver?.productId ?? product.markets.naver?.externalId)?.trim();
  if (!value || !/^\d+$/.test(value)) throw new Error("네이버 매핑에는 숫자형 원상품번호(originProductNo)가 필요합니다.");
  return value;
}

function mapStatus(status: string, stock: number | null): MarketState["saleStatus"] {
  if (status === "SUSPENSION" || status === "CLOSE" || status === "PROHIBITION") return "STOPPED";
  if (status === "OUTOFSTOCK" || stock === 0) return "OUT_OF_STOCK";
  if (status === "SALE") return "ON_SALE";
  return "UNKNOWN";
}

function hasManagedOptions(optionInfo: Record<string, unknown>): boolean {
  // Current Commerce API names are optionSimple, optionCustom, optionCombinations and optionStandards.
  // Sort/group metadata alone must never be treated as an actual stock-bearing option row.
  return ["optionSimple", "optionCustom", "optionCombinations", "optionStandards"]
    .some((key) => Array.isArray(optionInfo[key]) && (optionInfo[key] as unknown[]).length > 0);
}
